...
**Explicitly cut from v1** (deferred, not rebuilt now): Coach/AI chat screen (target of the *next* task after mobile ships, per your later message — foundation work now is just the capability seam, not the UI), Journal, Notes, Reports (view), Whiteboards (web-only permanently — Excalidraw has no viable native port), Sharing management, Library/templates, all admin screens.
