...
var taskStatusSchema = z2.enum(["BACKLOG", "TODO", "DOING", "DONE"]);
var createTaskSchema = z2.object({
  title: z2.string().min(1, "Title is required"),
  description: z2.string().optional(),
  category: z2.string().optional(),
  status: taskStatusSchema.optional(),
  estimatedMinutes: z2.number().min(1, "Estimated minutes must be at least 1").optional(),
  goalId: z2.string().uuid().optional(),
  scheduleBlockId: z2.string().uuid().optional(),
  dueDate: z2.string().optional(),
  notes: z2.string().optional()
});
var updateTaskSchema = createTaskSchema.partial();
var completeTaskSchema = z2.object({
  actualMinutes: z2.number().min(1, "Actual minutes must be at least 1"),
  notes: z2.string().optional(),
...
var createScheduleBlockSchema = z3.object({
  title: z3.string().min(1, "Title is required"),
  startTime: z3.string().regex(HH_MM_PATTERN, "Start time must be in HH:mm format"),
  endTime: z3.string().regex(HH_MM_PATTERN, "End time must be in HH:mm format"),
  dayOfWeek: z3.number().int().min(0).max(6),
  category: z3.string().min(1, "Category is required (value from the user's categories)"),
  color: z3.string().optional(),
  isRecurring: z3.boolean().optional(),
  isPrivate: z3.boolean().optional(),
  goalId: z3.string().uuid().optional(),
  seriesId: z3.string().uuid().optional()
});
var updateScheduleBlockSchema = createScheduleBlockSchema.partial().extend({
  updateScope: z3.enum(["single", "series"]).optional()
});

// src/validation/time-entry.ts
import { z as z4 } from "zod";
var createTimeEntrySchema = z4.object({
  taskName: z4.string().min(1, "Task name is required"),
  duration: z4.number().min(1, "Duration must be at least 1 minute"),
  date: z4.string().min(1, "Date is required"),
  notes: z4.string().optional(),
  startedAt: z4.string().optional(),
  taskTitle: z4.string().optional(),
  goalId: z4.string().uuid().optional(),
  scheduleBlockId: z4.string().uuid().optional(),
  taskId: z4.string().uuid().optional()
});
...
var upsertJournalEntrySchema = z5.object({
  date: z5.string().regex(YYYY_MM_DD, "Date must be YYYY-MM-DD"),
  content: z5.string().max(MAX_JOURNAL_CONTENT_LENGTH, "Entry is too long to save")
});
var updateJournalEntrySchema = z5.object({
  content: z5.string().max(MAX_JOURNAL_CONTENT_LENGTH, "Entry is too long to save")
});
...
function findUpcomingScheduleBlocks(weekSchedule, now, timezone, count) {
  ...
  for (let offset = 0; offset < 7 && out.length < count; offset++) {
    ...
    const targetCalendarDate = addDays2(calendarAnchor, offset);
...
function createTaskQueries(tasksApi) {
  ...
  return {
    ...
    fetchTasks,
    fetchTask,
    list: (filters) => queryOptions5({
      queryKey: taskQueries.list(filters),
      queryFn: () => fetchTasks(filters)
    }),
    detail: (id) => queryOptions5({
...
import { queryOptions as queryOptions9 } from "@tanstack/react-query";
...
import { queryOptions as queryOptions12 } from "@tanstack/react-query";
...
function createNotificationQueries(api) {
  ...
  return {
    ...
    infiniteList: (scope) => infiniteQueryOptions({
    ...
    unreadCount: (scope) => queryOptions15({
...
import { queryOptions as queryOptions16 } from "@tanstack/react-query";
